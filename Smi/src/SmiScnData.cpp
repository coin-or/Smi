...
SmiCoreData::SmiCoreData(OsiSolverInterface *osi, int nstag, int *cstag, int *rstag, int *integerIndices, int integerLength, int *binaryIndices, int binaryLength) : nrow_(0), ncol_(0), nz_(0), nstag_(nstag), nColInStage_(NULL), nRowInStage_(NULL), stageColPtr_(NULL), colStage_(NULL), rowStage_(NULL), colEx2In_(NULL), rowEx2In_(NULL),
...
SmiCoreData::SmiCoreData(CoinMpsIO *osi, int nstag, int *cstag, int *rstag, int *integerIndices, int integerLength, int *binaryIndices, int binaryLength) : nrow_(0), ncol_(0), nz_(0), nstag_(nstag), nColInStage_(NULL), nRowInStage_(NULL), stageColPtr_(NULL), colStage_(NULL), rowStage_(NULL), colEx2In_(NULL), rowEx2In_(NULL),
																																							colIn2Ex_(NULL), rowIn2Ex_(NULL), integerIndices_(NULL), integerLength_(0), binaryIndices_(NULL), binaryLength_(0),
																																							cdrlo_(NULL), cdrup_(NULL), cdobj_(NULL), cdclo_(NULL), cdcup_(NULL), nodes_(), pDenseRow_(), intColsStagewise(nstag, std::vector<int>()), colNamesStrict(NULL), colNamesFree(NULL)
{
	int nrow = osi->getNumRows();
...
SmiNodeData::SmiNodeData(SmiStageIndex stg, SmiCoreData *core,
						 ...
						 CoinPackedVector *drup) : stg_(stg),
												   ...
												   ptr_count(0) // used for counted pointer
{
	...
	if (matrix && matrix->getNumElements() > 0)
	{
		...
		const CoinPackedMatrix *localMatrix = (matrix->isColOrdered() ? revOrdMatrix : matrix);
