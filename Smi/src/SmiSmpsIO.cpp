...
SmiSmpsIO::readTimeFile(SmiScnModel *smi, const char *c, const char *ext)
{
	...
	if (smpsCardReader_->whichSection() == COIN_NAME_SECTION)
	{
		...
		{
			printf(
					"Warning: Time file name %s does not match problem file name %s\n",
	...
	else if (smpsCardReader_->whichSection() == COIN_UNKNOWN_SECTION)
	{
		handler_->message(COIN_MPS_BADFILE1, messages_)
				<< smpsCardReader_->card() << 1 << fileName_ << CoinMessageEol;
		/*
		 #ifdef COIN_HAS_ZLIB
	...
	if (iftime)
	{
		...
		if (smpsCardReader_->whichSmpsSection() == SMI_ENDATA_SECTION
				&& smpsCardReader_->whichSmpsType()
						== SMI_TIME_ORDERED_CORE_TYPE)
		{
			int i;
...
int SmiSmpsIO::readStochFile(SmiScnModel *smi, SmiCoreData *core, const char *c,
		const char *ext)
{
...
SmiSectionType SmiSmpsCardReader::nextSmpsField()
{
	...
	while (!gotCard)
	{
		...
		else if (card_[0] != '*')
		{
			...
			handler_->message(COIN_MPS_LINE, messages_) << cardNumber_ << card_
					<< CoinMessageEol;
			...
			if (i == SMI_UNKNOWN_SECTION)
				return (SmiSectionType) i;
					...
					|| (smiSection_ == SMI_INDEPENDENT_SECTION))
			{
				...
				{
					for (i = SMI_SMPS_COMBINE_ADD; i < SMI_SMPS_COMBINE_UNKNOWN;
							i++)
					{
...
SmiSmpsCardReader::~SmiSmpsCardReader()
{
	if (combineRuleSet)
	{
		SmiCoreCombineReplace * ruleReplace =
				dynamic_cast<SmiCoreCombineReplace*>(combineRule_);
		SmiCoreCombineAdd * ruleAdd =
				dynamic_cast<SmiCoreCombineAdd*>(combineRule_);
		if (ruleReplace)
